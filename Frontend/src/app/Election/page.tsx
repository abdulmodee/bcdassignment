...
export default function ElectionsPage() {
  ...
  const handleEndElection = async (
    ...
    try {
      ...
      setStatus(
        `Election "${
          myElections.find((e) => e.address === electionAddress)?.title
        }" ended successfully`
      );
      setStatusType('success');
    } catch (err: any) {
                ...
                {loadingElections ? (
                    ...
                    {myElections.map((election) => (
                      ...
                      >
                        <HostCard
                          ...
                          onEndElection={() =>
                            handleEndElection(election.address!, election.id)
                          }
                          voteData={election.candidates.map((c) => ({
                            name: c.name,
                            votes: c.votes || 0,
                          }))}
                          // voterMetrics={{
                  ...
                  >
                    ...
                    <Typography variant="body1" color="#aaa" mb={3}>
                      Create your first election by clicking the "Create New
                      Election" tab
                    </Typography>
                    <Button
                      variant="contained"
