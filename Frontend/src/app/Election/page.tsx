...
export default function ElectionsPage() {
                ...
                for (let i = 0; i < electionAddresses.length; i++) {
                    try {
                        ...
                        if (owner.toLowerCase() === account.toLowerCase()) {
                            ...
                            const totalVotes: number = candidates.reduce((sum: number, candidate: Candidate) => sum + (candidate.votes || 0), 0);
        ...
        try {
            ...
            if (electionAddress) {
                setStatus(`Election created successfully at address ${electionAddress}. 
                         Voting will start ${startDateTime.format('MMM D, YYYY [at] h:mm A')} 
                         and end ${endDateTime.format('MMM D, YYYY [at] h:mm A')}`);
    ...
    const handleEndElection = async (electionAddress: string, electionId: number) => {
        ...
        try {
            ...
            setMyElections(prev => prev.map(election =>
                election.address === electionAddress
                    ? { ...election, ended: true }
                    : election
            ));

            setStatus(`Election "${myElections.find(e => e.address === electionAddress)?.title}" ended successfully`);
            setStatusType('success');
        } catch (err: any) {
                                ...
                                <Box component="form" onSubmit={handleElection} noValidate>
                                    ...
                                    <Box sx={{ mb: 6 }}>
                                        ...
                                        {startDateTime && endDateTime && (
                                            <Box sx={{ mt: 2, px: 1 }}>
                                                <Typography variant="body2" color={dayjs(endDateTime).diff(dayjs(startDateTime), 'hour') < 1 ? '#f44336' : '#00c896'}>
                                                    Election period: {calculatePeriod(startDateTime, endDateTime)}
                                                </Typography>
                                            </Box>
                                        )}
                                    </Box>
